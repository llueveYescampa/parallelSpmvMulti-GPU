...
/////////////////////////////////////////////////////////////////////////
    ...
    for (int t=0; t<REP; ++t) {
        ...
        for (int gpu=0; gpu<ngpus; ++gpu) {
            ...
            cuda_ret = cudaMemsetAsync(w_d[gpu], 0, sizeof(real)*n[gpu],stream0[gpu] );
            ...
            spmv<<<grid0[gpu], block0[gpu], sharedMemorySize0[gpu],stream0[gpu]>>>(w_d[gpu],  row_ptr_d[gpu], col_idx_d[gpu], n[gpu]);
        ...
        if (ngpus > 1 ) {
            ...
            for (int gpu=0; gpu<ngpus; ++gpu) {
                ...
                spmv<<<grid1[gpu], block1[gpu], sharedMemorySize1[gpu],stream1[gpu] >>>(w_d[gpu],  row_ptr_off_d[gpu], col_idx_off_d[gpu], n[gpu]);
                cuda_ret = cudaUnbindTexture(xTex);
                cuda_ret = cudaUnbindTexture(valTex);
